  ...
  angular.module('builder.controller', ['builder.provider']).controller('fbFormObjectEditableController', [
    '$scope', '$injector', function($scope, $injector) {
      ...
      $scope.setupScope = function(formObject) {
        ...
        $scope.$watch('[label, description, placeholder, required, options, fieldName, logicRules, prepend, append, appendOrPrependClass, draggable, validation]', function() {
          ...
          formObject.logicRules = $scope.logicRules;
          formObject.prepend = $scope.prepend;
          formObject.append = $scope.append;
          formObject.appendOrPrependClass = $scope.appendOrPrependClass;
          formObject.draggable = $scope.draggable;
      ...
      return $scope.data = {
        ...
        backup: function() {
          ...
          return this.model = {
			...
			append: $scope.append,
			appendOrPrependClass: $scope.appendOrPrependClass,
			draggable: $scope.draggable,
            logicRules: $scope.logicRules
          };
        },
        rollback: function() {
		  ...
		  $scope.prepend = this.model.prepend;
          $scope.append = this.model.append;
          $scope.appendOrPrependClass = this.model.appendOrPrependClass;
          $scope.draggable = this.model.draggable;
          $scope.logicRules = this.model.logicRules;
    ...
    '$scope', '$injector', function($scope, $injector) {
	  ...
	  $scope.addsubfields = function() {
		console.log('add more clicked');
	  };
	  $scope.delete_preview = function(item) {
		item.inputText = false;
		var fieldIndex = item.index;
		$scope.input[fieldIndex].value = '';
		$('.close_show_' + $scope.formName + '_' +fieldIndex).removeClass('show').addClass('hide');
		$('.custom_attachment_' + $scope.formName + '_' +fieldIndex).attr('src',item.AWS_BASE_URL + '/Channel/0/default.png');
		$scope.$apply();
	  };
    ...
    '$scope', '$injector', function($scope, $injector) {
      ...
      return $scope.updateInput = function(value) {
        ...
        input = {
		  ...
		  fieldIndex: $scope.formObject.index,
...
(function() {
  angular.module('builder.directive', ['builder.provider', 'builder.controller', 'builder.drag', 'validator']).directive('fbBuilder', [
      ...
      return {
        ...
        link: function(scope, element) {
          ...
          scope.popover = {
            save: function($event) {
			  var data = {};
			  data.key = scope.fieldName;
			  data.label = scope.label;
			  data.required = scope.required;
			  data.instructions = scope.description;
			  data.placeholder = scope.placeholder;
              ...
              $event.preventDefault();
			  /*
			  AcfUpdateFactory.edit(data, function(response) {
			  });*/
      ...
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          if ((_base = $builder.forms)[_name = scope.formName] == null) {
			...
			scope.conditionalLogic = function(logicRules) {
				if (logicRules.status == 0) {
					return true;
				}
				var match = 0;
				$.each(logicRules.rules, function(i, ruleField) {
					$.each(scope.input, function(j, inputField) {
						if (inputField.fieldName == ruleField.field) {
							if (ruleField.operator == '==' && inputField.value == ruleField.value) {
								match++;
								return;
							} else if (ruleField.operator == '!=' && inputField.value != ruleField.value) {
								match++;
								return;
							}
						}
				...
				if (logicRules.allorany == 'all' && match == logicRules.rules.length) {
					return true;
				} else if (logicRules.allorany == 'any' && match) {
					return true;
				}
			};
			scope.fileAttached = function(element) {
				var url = api_url + 'api/v' + version +  '/image_upload.json?token='+token+'&domain_id='+domain_id;
				$(element).fileupload({
					method: 'POST',
		            url: url,
		            dropZone: $('.drop_files'),
				...
				field_name = field_name.replace('_file[]', '');
				data.formData = {file_field_name: field_name, type : 'additional', user_id : Auth.id};
				data.files = element.files[0];
				$(element).fileupload('send', data);
				$(element).on('fileuploaddone', function (e,response) {
					if(angular.isDefined(response)){
						var fieldIndex = $(element).data('index');
						$('.custom_attachment_' + scope.formName + '_' + fieldIndex).attr('src',response.result.picture_url);
						$('.close_show_' + scope.formName + '_' + fieldIndex).removeClass('hide').addClass('show');
						scope.input[fieldIndex].value = response.result.filename;
					}
				});
			}
    ...
    '$injector', function($injector) {
      var $builder, $compile, $parse, $rootScope;
      $builder = $injector.get('$builder');
      $compile = $injector.get('$compile');
      ...
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          scope.$component = $builder.components[scope.formObject.component];
		  scope.AWS_BASE_URL = $rootScope.AWS_BASE_URL;
  ...
  angular.module('builder.provider', []).provider('$builder', function() {
    ...
    this.convertComponent = function(name, component) {
      ...
      result = {
        ...
        fieldName: (_ref10 = component.fieldName) != null ? _ref10 : false,
        logicRules: (_ref11 = component.logicRules) != null ? _ref11 : false,
        prepend: (_ref11 = component.prepend) != null ? _ref12 : false,
        append: (_ref11 = component.append) != null ? _ref13 : false,
        appendOrPrependClass: (_ref11 = component.appendOrPrependClass) != null ? _ref14 : false,
        draggable: (_ref11 = component.draggable) != null ? _ref15 : false,
    ...
    this.convertFormObject = function(name, formObject) {
      var component, exist, form, result, _i, _len, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _ref9, _ref10, _ref11, _ref12, _ref13, _ref14, _ref15;
      if (formObject == null) {
        formObject = {};
      }
      ...
      result = {
        ...
        fieldName: (_ref10 = formObject.fieldName) != null ? _ref10 : component.fieldName,
        logicRules: (_ref11 = formObject.logicRules) != null ? _ref11 : component.logicRules,
		prepend: (_ref11 = formObject.prepend) != null ? _ref12 : component.prepend,
        append: (_ref11 = formObject.append) != null ? _ref13 : component.append,
        appendOrPrependClass: (_ref11 = formObject.appendOrPrependClass) != null ? _ref14 : component.appendOrPrependClass,
        draggable: (_ref11 = formObject.draggable) != null ? _ref15 : component.draggable
      };
      return result;
    };
