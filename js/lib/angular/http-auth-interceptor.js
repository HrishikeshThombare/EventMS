...
(function () {
  ...
  .config(['$httpProvider', function($httpProvider) {
    $httpProvider.interceptors.push(['$rootScope', '$q', 'httpBuffer','$location', 'SessionService',function($rootScope, $q, httpBuffer, $location, SessionService) {
      return {
            ...
            switch (rejection.status) {
              case 401:
				...
				if(refresh_token === null || refresh_token === '' || refresh_token === undefined){
					...
					$.removeCookie('isUniversalAdmin');
					$.removeCookie('isChannelAdmin');
					$.removeCookie('isNetworkAdmin');
					$.removeCookie('isMDNAdmin');
					$.removeCookie('isUser');
					$.removeCookie('isCurator');
					var redirectto = $location.absUrl().split('/#!/');
					var find_admin = redirectto[0].split('/');
					if(find_admin[find_admin.length - 1] === 'ag-admin'){
						redirectto =  redirectto[0] + '/#!/admin/users/login';
					} else {
